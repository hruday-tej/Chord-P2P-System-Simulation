...
let akkaSystem = System.create "akkaSystem" <| Configuration.load ()
let akkaRouting = "akka://akkaSystem/user/"
...
module randomGenerationUtilityModule = 
    ...
    let nRandomForId (num : int) : bigint[] =
        Array.init num (fun _ ->
            ...
            let result = 
                hashed
                |> Array.rev
                |> Array.fold (fun acc byteVal ->
                    acc * 256I + bigint(byteVal)
                ) 0I
            result
        )
    ...
    let thePowerOfTwo (n : int) : bigint =
        bigint(Math.Pow(2.0, float n))



module mainFunction = 
    let BossActorRef = akkaSystem.ActorSelection(akkaRouting+"BossActor")
    ...
    let chordProtocol (mailbox: Actor<_>) = 
        ...
        let rec loop() = actor{
            ...
            | PeerCreation (idBits, N, isFirst) ->
                n <- N
                m <- randomGenerationUtilityModule.thePowerOfTwo idBits
                idLen <- idBits
            ...
            | SuccessorFind(peerStart, id, queryNo, fingerIndex, hops) ->
                ...
                if foundIt then
                    let originPeerRef = akkaSystem.ActorSelection(akkaRouting + string(peerStart))
                    ...
                    let succPeer = akkaSystem.ActorSelection(akkaRouting + string(nextChild))
            ...
            | PredecessorMsgReceive(predecessor) ->
                ...
                if changeInSuccessor then
                    child <- predecessor
                let succPeer = akkaSystem.ActorSelection(akkaRouting + string(child))
                succPeer <! Alert(n)
            ...
            | Join(chordPeer) ->
                let chordPeer = akkaSystem.ActorSelection(akkaRouting + string(chordPeer))
                chordPeer <! SuccessorFind(n, n, 1, 0, 0)
                
            | Maintain ->
                if child <> dummy then
                    let succ = akkaSystem.ActorSelection(akkaRouting + string(child))
                    succ <! SendMsgPredecessor   

            | StartMaintenance ->
                let stab = async{
                    while true do
                        let peerActor = akkaSystem.ActorSelection(akkaRouting + string(n))
                        peerActor <! Maintain
            ...
            | FingersToBeFixed ->
                if child <> dummy then
                    ...
                    let mutable v = randomGenerationUtilityModule.thePowerOfTwo (next - 1)
                    v <- (n + v) % m

                    let path = akkaRouting + string(n)
                    let currPeer = akkaSystem.ActorSelection(path)
...
let BossActorRef =  spawn akkaSystem "BossActor" <| mainFunction.bossActor 
BossActorRef <! BossActorInit(noOfPeers, noOfRequests)
let mutable peers = []
    ...
    let maintain = async {
        while true do
            let tasks =
                ...
                |> List.map (fun peer ->
                    async {
                        let peerActor = akkaSystem.ActorSelection(akkaRouting + peer)
                        peerActor <! Maintain
                    }
                )
    ...
    let updateFinger = async {
        while true do
            let tasks =
                ...
                |> List.map (fun peer ->
                    async {
                        let peerActorRef = akkaSystem.ActorSelection(akkaRouting + peer)
...
module actionSet2 = 
    ...
    let keyFinding (peerIdentifier, key)  = 
        let peerRef = akkaSystem.ActorSelection(akkaRouting + string(peerIdentifier))
        peerRef <! SuccessorFind(peerIdentifier, key, (Math.Pow(2.0, 1.0)|>int) + 1, 0, 0)


module finalFunc =
    let mutable x = 1
    let nextPeer = randomGenerationUtilityModule.nRandomForId noOfPeers
    
    let peerActor = spawn akkaSystem (string(nextPeer.[0])) <| mainFunction.chordProtocol
    peers <- peers @ [string(nextPeer.[0])]
    firstChordPeer <- nextPeer.[0]
    peerActor <! PeerCreation((Math.Pow(2.0, 4.0)|>int) * 10, nextPeer.[0], true)
    peerActor <! ReqCount(noOfRequests) 
    
    for i in 1..nextPeer.Length - 1 do
        let peerActor = spawn akkaSystem (string(nextPeer.[i])) <| mainFunction.chordProtocol
    ...
    let requests = randomGenerationUtilityModule.nRandomForId noOfRequests
    
    for i in 0..nextPeer.Length - 1 do
        let peerRef = akkaSystem.ActorSelection(akkaRouting + string(nextPeer.[i]))
